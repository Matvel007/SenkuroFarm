<div align="center">

<img src="logo.png" width="150" alt="Логотип Senkuro Farm">

# ⚡ Senkuro Farm

**Android-приложение для просмотра коллекции карт, статистики и автоматической прокрутки страниц Senkuro**

[![Android](https://img.shields.io/badge/Android-7.0%2B-3DDC84?logo=android&logoColor=white)](https://www.android.com/)
[![Kotlin](https://img.shields.io/badge/Kotlin-Jetpack%20Compose-7F52FF?logo=kotlin&logoColor=white)](https://kotlinlang.org/)
[![Version](https://img.shields.io/badge/version-1.1.0-9B7BFF)](https://github.com/Matvel007/SenkuroFarm/releases)
[![License](https://img.shields.io/badge/license-MIT-blue.svg)](#-лицензия)

</div>

> [!IMPORTANT]
> Senkuro Farm — неофициальное приложение. Проект не связан с администрацией Senkuro. Используя приложение, вы самостоятельно отвечаете за соблюдение правил сайта.

## 📱 Скриншоты

<div align="center">
  <img src="4.jpg" width="205" alt="Экран приветствия">
  <img src="3.jpg" width="205" alt="Экран фарма">
  <img src="1.jpg" width="205" alt="Статистика">
  <img src="2.jpg" width="205" alt="Настройки">
</div>

## ✨ Возможности

- 🃏 Просмотр коллекции карт аккаунта в удобной сетке.
- 💎 Корректное отображение обычных карт и осколков.
- ⚡ Автоматическая прокрутка страниц выбранной книги.
- 🎁 Обнаружение и автоматический сбор выпавших карточек.
- 🌙 Работа в фоне и при выключенном экране через `ForegroundService`.
- 📊 Статистика времени работы, коллекции по рангам и карт, собранных ботом.
- 🧠 Кэширование коллекции для быстрой и плавной прокрутки.
- 🔔 Постоянное уведомление с состоянием фарма и кнопкой остановки.
- 🎨 Тёмное минималистичное оформление на Jetpack Compose.
- 🔐 Авторизация через WebView — пароль приложением не сохраняется.
- 🔄 Автоматическая проверка обновлений через GitHub Releases.

## 🚀 Установка

1. Скачайте APK из раздела **Releases**.
2. Разрешите установку приложений из выбранного источника.
3. Установите APK и запустите Senkuro Farm.

Минимальная версия системы — **Android 7.0 (API 24)**.

> [!NOTE]
> Для обновления устанавливайте новую версию поверх предыдущей. Если Android сообщает о несовместимой подписи, удалите старую тестовую/debug-сборку и установите release-версию заново.

## 📖 Использование

1. На стартовом экране нажмите **«Выдать разрешение»** и разрешите приложению отображаться поверх других окон.
2. Нажмите **«Войти через Senkuro»**.
3. Авторизуйтесь на открывшемся сайте.
4. После успешного входа нажмите **«Я вошёл»** в верхней части экрана.
5. Откройте вкладку **«Фарм»** и перейдите к нужной книге и главе.
6. Выберите скорость прокрутки и нажмите **«Старт»**.
7. Приложение продолжит работу в фоне. Остановить фарм можно внутри приложения или кнопкой в уведомлении.

> [!WARNING]
> Разрешение **«Поверх других окон»** обязательно для фоновой WebView. На Xiaomi, Poco и некоторых эмуляторах также рекомендуется разрешить автозапуск и отключить ограничения энергосбережения для приложения.

## 🧭 Разделы приложения

| Раздел | Назначение |
|---|---|
| 🏠 **Карты** | Коллекция аккаунта, ранги, количество карт и осколки |
| ⚡ **Фарм** | Управление прокруткой, скорость, текущий URL и состояние |
| 📈 **Статы** | Общее время работы и статистика карт |
| ⚙️ **Настройки** | Тема, уведомления, батарея, автозапуск и системные разрешения |
| ℹ️ **Инфо** | Версия, лицензия и информация о проекте |

## 🛠️ Технологии

- Kotlin
- Jetpack Compose + Material 3
- Android WebView
- Foreground Service + WakeLock
- OkHttp
- Coil
- DataStore
- Navigation Compose

## 🧑‍💻 Сборка из исходников

Понадобятся Android Studio, Android SDK и JDK 11 или новее.

```bash
git clone https://github.com/Matvel007/SenkuroFarm.git
cd SenkuroFarm
./gradlew assembleDebug
```

Debug APK появится здесь:

```text
app/build/outputs/apk/debug/app-debug.apk
```

Для release-сборки создайте собственный ключ подписи и файл `keystore.properties`. Ключи и пароли не должны попадать в Git.

## 🔒 Конфиденциальность

- Пароль пользователя не сохраняется приложением.
- Авторизация выполняется непосредственно в WebView сайта Senkuro.
- Сессионные cookies хранятся локально на устройстве.
- Приложение не передаёт учётные данные сторонним сервисам.

## ⚖️ Отказ от ответственности

Все права на название, торговые марки и контент Senkuro принадлежат их законным владельцам. Приложение использует доступный веб-интерфейс сайта и предоставляется «как есть», без гарантий стабильности или совместимости с будущими изменениями Senkuro.

## 📄 Лицензия

Исходный код распространяется на условиях [MIT License](LICENSE).

<div align="center">

Сделано для Android с использованием Kotlin и Jetpack Compose.

</div>
